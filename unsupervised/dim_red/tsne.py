...
class TSNE:
    def grid_search(self, diff_i, i, perplexity):
        '''
    ...
    def get_original_pairwise_affinities(self, X: np.array([]),
        ...
        for i in range(0, n):
            # Equation 1 numerator
            diff = X[i] - X
            σ_i = self.grid_search(diff, i, perplexity)  # Grid Search for σ_i
    ...
    def low_dimensional_embedding(self, p_ij: np.array([]),
                                  Y0: np.array([]),
                                  T=1000,
                                  η=200,
        ...
        for t in range(1, T - 1):
            ...
            gradient = self.get_gradient(early_exaggeration * p_ij, q_ij, Y[t])
    ...
    def fit_transform(self, X: np.array([]),
        ...
        p_ij = self.get_original_pairwise_affinities(X, perplexity)
        p_ij_symmetric = self.get_symmetric_p_ij(p_ij)

        # Initialization
        Y = np.zeros(shape=(T, n, n_dimensions))
        Y_minus1 = np.zeros(shape=(n, n_dimensions))
        Y[0] = Y_minus1
        Y1 = self.initialization(X, n_dimensions)
        Y[1] = np.array(Y1)
